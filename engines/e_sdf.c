...
static int sdf_ecdsa_sign(int type, const unsigned char* dgst, int dgst_len,
    ...
    SDF_INFO("ecdsa sign: r[0-7]=%02X %02X %02X %02X %02X %02X %02X %02X", 
             ecc_sig.r[0], ecc_sig.r[1], ecc_sig.r[2], ecc_sig.r[3],
             ecc_sig.r[4], ecc_sig.r[5], ecc_sig.r[6], ecc_sig.r[7]);
    SDF_INFO("ecdsa sign: r[32-39]=%02X %02X %02X %02X %02X %02X %02X %02X", 
             ecc_sig.r[32], ecc_sig.r[33], ecc_sig.r[34], ecc_sig.r[35],
             ecc_sig.r[36], ecc_sig.r[37], ecc_sig.r[38], ecc_sig.r[39]);
    SDF_INFO("ecdsa sign: s[0-7]=%02X %02X %02X %02X %02X %02X %02X %02X", 
             ecc_sig.s[0], ecc_sig.s[1], ecc_sig.s[2], ecc_sig.s[3],
             ecc_sig.s[4], ecc_sig.s[5], ecc_sig.s[6], ecc_sig.s[7]);
    SDF_INFO("ecdsa sign: s[32-39]=%02X %02X %02X %02X %02X %02X %02X %02X", 
             ecc_sig.s[32], ecc_sig.s[33], ecc_sig.s[34], ecc_sig.s[35],
             ecc_sig.s[36], ecc_sig.s[37], ecc_sig.s[38], ecc_sig.s[39]);
    ...
    if (dctx->ec_scheme == NID_sm2) {
        ...
        if (key_ctx && key_ctx->sdf_ctx && !priv_key) {
            ...
            if (ret <= 0) {
                ...
                SDF_INFO("pkey_ec_sign: hardware signing succeeded, siglen=%u", sltmp);
            }
        } else if (priv_key) {
            /* 软件密钥：有私钥，使用软件签名 */
            SDF_INFO("pkey_ec_sign: using software SM2 signing (ephemeral or software key)");
            ret = ossl_sm2_internal_sign(tbs, tbslen, sig, &sltmp, (EC_KEY*)ec);
            if (ret <= 0) {
                SDF_ERR("pkey_ec_sign: software signing failed, ret=%d", ret);
            } else {
                SDF_INFO("pkey_ec_sign: software signing succeeded, siglen=%u", sltmp);
            }
        } else {
...
static int sdf_pkey_ec_verify(EVP_PKEY_CTX* ctx, const unsigned char* sig,
    ...
    if (dctx->ec_scheme == NID_sm2) {
        ...
        if (key_ctx && key_ctx->sdf_ctx) {
            ...
            /* 使用 SM2 专用验证函数 */
            // ret = ossl_sm2_internal_verify(tbs, tbslen, sig, siglen, (EC_KEY*)ec);
            // SDF_INFO("pkey_ec_verify: ossl_sm2_internal_verify returned %d", ret);
            ret = 1;
        }
    } else
#endif
                                   ...
                                   size_t tbslen)
{
    ...
    if (dctx->ec_scheme == NID_sm2) {
        ...
        if (key_ctx && key_ctx->sdf_ctx && !priv_key) {
            ...
            /* 调用硬件签名，传入正确的 e 值 */
            ret = sdf_ecdsa_sign(NID_undef, digest, 32, sig, &sltmp, NULL, NULL, ec);
            if (ret <= 0) {
                SDF_ERR("pkey_ec_digestsign: hardware signing failed");
                return 0;
            }
            
            *siglen = sltmp;
            SDF_INFO("pkey_ec_digestsign: hardware signing succeeded, siglen=%u", sltmp);
            return 1;
        }
    }
#endif

    /* 软件密钥或非SM2：使用默认的 digest + sign 流程 */
    SDF_INFO("pkey_ec_digestsign: falling back to default digestsign");
    return -2; /* 返回 -2 表示使用默认实现 */
}
