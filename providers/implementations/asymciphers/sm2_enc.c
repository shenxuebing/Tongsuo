                            ...
                            size_t inlen)
{
    ...
    return ossl_sm2_encrypt(psm2ctx->key, md, in, inlen, out, outlen, 0);
}
                            ...
                            size_t inlen)
{
    ...
    if (out == NULL) {
        if (!ossl_sm2_plaintext_size(in, inlen, outlen, 0))
            return 0;
        return 1;
    }

    return ossl_sm2_decrypt(psm2ctx->key, md, in, inlen, out, outlen, 0);
}
