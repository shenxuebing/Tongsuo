...
/* Note: Removed global variable approach for passing ENGINE-generated ephemeral public key.
 * Now using EVP_PKEY_CTX_ctrl to query the key directly from ENGINE when needed.
 */
...
int ssl_derive_ntls(SSL *s, EVP_PKEY *privkey, EVP_PKEY *pubkey, int gensecret)
{
    ...
    if (use_engine_sm2dhe) {
         ...
         * We need to get the ENGINE-generated ephemeral public key and save it to SSL structure
         * for use in tls_construct_cke_sm2dhe_ntls (which is called BEFORE this function returns).
         * 
         * Note: For responder, the ENGINE generates the ephemeral key pair during EVP_PKEY_derive
         * (inside SDF_GenerateAgreementDataAndKeyWithECCEx). We retrieve it AFTER derive completes.
         */
        if (!s->server) {
            struct {
                unsigned char **pub;
